...
create_sanky_plot <- function(plot_parameters) {
  ...
  for (comparison in names(questionnaire_comparisons)) {
    ...
    averages <- c(averages, paste0(getFormattedAverage(first_responses), ", ", getFormattedAverage(second_responses)))
  ...
  comparison_results <- data.frame(
    ...
    effect_interpretation_values, values, averages, test_warnings, q_values,
