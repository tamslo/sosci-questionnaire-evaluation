...
getEvolutionTableData <- function() {
  filterEvolutionTableRows <- function(data) {
    # TODO for questions included in evolution table, include all time points
    # TODO only reported numbers for HPI ST 2020; also report ST 2021 and TUM?
    return(data[which(
      data$pValue != "–" &
        data$pValue <= EVOLUTION_SUPPLEMENT_THRESHOLD &
        !startsWith(data$questionTitle, "I want to (Q1, Q2) or did (Q3) ") & # redundant Q1-Q3
  ...
  sortEvolutionTable <- function(data) {
    # TODO time points per question should be ordered (in general, order by question?)
    return(data[order(data$pValue),])
  }
