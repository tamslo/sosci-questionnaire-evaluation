...
getCleanQuestionnaire <- function(questionnaire) {
  if (!str_detect(questionnaire, fixed("_"))) {
     return(questionnaire)
    } else if (questionnaire == "TUM_Retrospective") {
      return("Q4")
    } else { # questionnaire name with uni name and underscore
     return(str_remove(str_remove(str_remove(questionnaire, TUM_NAME), HPI_NAME), "_"))
    }
}
...
combineCompleteResultData <- function() {
  ...
  for (rowIndex in 1:nrow(results)) {
    ...
    # Get CLEAN_QUESTNNR
    CLEAN_QUESTNNR <- c(CLEAN_QUESTNNR, getCleanQuestionnaire(questionnaire))
    # Get COURSE_YEAR
...
getEvolutionStats <- function() {
    ...
    createStackedBarplotsForUniAndYear <- function(parameters) {
      ...
      for (questionName in names(plotParameters)) {
        ...
        for (comparisonName in names(comparisons)) {
          ...
          comparisonStatistics[[paste(questionId, comparisonName)]] <- c(title, comparisonName, firstQuestionId, secondQuestionId, stats, values)
      ...
      colnames(comparisonStatistics) <- c("question.text", "comparison", "first.question.id", "second.question.id", statisticColumnNames, "values")
  ...
  for (statsKey in names(evolutionStatsParams)) {
    ...
    if (paired) {
      ...
      alternativePValue <- c(alternativePValue, stats[["alternative_p_values"]])
      alternativeEffectSize <- c(alternativeEffectSize, stats[["alternative_effect_values"]])
      alternativeEffectSizeInterpretation <- c(alternativeEffectSizeInterpretation, stats[["alternative_effect_interpretation_values"]])
    } else {
  ...
  evolution_stats <- data.frame(questionTitle, uni, year, comparison, firstQuestionId, secondQuestionId, isPaired, isBinary, pValue, effectSize, effectSizeInterpretation, values, alternativePValue, alternativeEffectSize, alternativeEffectSizeInterpretation)
...
EVOLUTION_STATS <- runIfNotPresent(getEvolutionStats, EVOLUTION_STATS_PATH, forceRun = TRUE, readOutput = read.csv)
...
getAggregatedData <- function() {
  ...
  source("modules/view/plotting/label.R")
...
getEvolutionTableData <- function() {
  tableData <- subset(EVOLUTION_STATS[which(
  EVOLUTION_STATS$pValue != "—" &
    EVOLUTION_STATS$pValue <= 0.1 &
    !startsWith(EVOLUTION_STATS$questionTitle, "I want to (Q1, Q2) or did (Q3) ") & # redundant Q1-Q3
    (EVOLUTION_STATS$uni != HPI_NAME | EVOLUTION_STATS$uni == HPI_NAME & EVOLUTION_STATS$isPaired) &
    (EVOLUTION_STATS$year != HPI_FIRST_YEAR | EVOLUTION_STATS$year == HPI_FIRST_YEAR & EVOLUTION_STATS$isBinary)
  ),], select = -c(firstQuestionId, secondQuestionId, alternativePValue, alternativeEffectSize, alternativeEffectSizeInterpretation))
...
renderTable(EVOLUTION_TABLE_DATA)
...
option_counts <- as.data.frame(table(COMPLETE_OPTIONS$VAR))
names(option_counts) <- c("VAR", "COUNT")
binary_question_ids <- as.vector(option_counts$VAR[which(option_counts$COUNT == 2)])
print(binary_question_ids)
for (question_id in unique(COMPLETE_OPTIONS$VAR)) {
  options <- COMPLETE_OPTIONS[which(COMPLETE_OPTIONS$VAR == question_id),]$MEANING
  if (("Yes" %in% options && "No" %in% options) || ("Ja" %in% options && "Nein" %in% options)) {
    binary_question_ids <- c(binary_question_ids, question_id)
  }
}
binary_evolutions <- EVOLUTION_STATS[which(EVOLUTION_STATS$firstQuestionId %in% binary_question_ids & EVOLUTION_STATS$secondQuestionId %in% binary_question_ids),]
# TODO: not all values for McNemar present (need better check!)
# TODO: double-check test values and inform group
# TODO: maybe adapt main binary paired test to be McNemar (better condition what is binary!)
```

```{r review-comment-better-evolution, include=FALSE}
# TODO
```
