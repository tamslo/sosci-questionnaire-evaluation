...
getEvolutionStats <- function() {
  ...
  evolution_stats <- data.frame(questionTitle, uni, year, comparison, firstQuestionId, secondQuestionId, isPaired, isBinary, pValue, testMethod, effectSize, effectSizeInterpretation, values, alternativePValue, alternativeTestMethod, alternativeEffectSize, alternativeEffectSizeInterpretation)
...
getEvolutionTableData <- function() {
  ...
  EVOLUTION_STATS$pValue != "–" &
    EVOLUTION_STATS$pValue <= EVOLUTION_SUPPLEMENT_THRESHOLD &
  ...
  ),], select = -c(firstQuestionId, secondQuestionId, alternativePValue, alternativeEffectSize, alternativeEffectSizeInterpretation, alternativeTestMethod, isBinary, isPaired))
  tableData <- tableData[order(tableData$pValue),]
...
source("modules/statistics/utils.R", local = TRUE)
getWilcoxonMccNemarDifferences <- function() {
...
WILCOXON_MCNEMAR_DIFFERENCES <- getWilcoxonMccNemarDifferences()
```
