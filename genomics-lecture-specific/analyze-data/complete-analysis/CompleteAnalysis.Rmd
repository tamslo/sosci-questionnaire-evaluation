...
printOpenEndedData <- function() {
  source("assets/combineResultData.R", local = TRUE)
  getQuestionResponses <- function(openQuestions, resultData) {
    ...
    for (questionIndex in 1:nrow(openQuestions)) {
      ...
      for (resultIndex in 1:nrow(resultData)) {
        ...
        if (!is.null(text) && text != "") {
          responses[[responseIndex]] <- list("questionnnaire" = response$QUESTNNR, "text" = text)
  ...
  questionResponses <- getQuestionResponses(COMPLETE_QUESTIONS[which(COMPLETE_QUESTIONS$type == "OPEN"),], COMPLETE_RESULTS)
  ...
  for (data_directory in c(INITIAL_VERSION_DIRECTORIES, CONFLICTING_VERSION_DIRECTORIES)) {
    ...
    open_questions <- question_data[which(question_data$id %in% missing_question_ids),]
    if (nrow(open_questions) > 0) {
      moreResponses <- getQuestionResponses(open_questions, result_data)
  ...
  for (questionId in names(questionResponses)) {
    ...
    for (responseIndex in names(responses)) {
      print(responseIndex)
      response <- responses[[responseIndex]]
      print(paste0("* [", response[["questionnaire"]], "] ", response[["text"]]))
    }
  }
