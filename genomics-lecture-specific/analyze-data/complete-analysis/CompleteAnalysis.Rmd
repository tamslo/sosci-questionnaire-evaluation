...
cleanEvolutionTableData <- function(data) {
    setNa <- function(data, colName, value) {
      indices <- which(data[[colName]] == value)
      data[indices, colName] <- NA
      return(data)
    }
    replaceSubstring <- function(data, colName, pattern, replacement) {
      data[, colName] <- unlist(lapply(cleanData[, colName], function(value) {
        str_replace_all(value, pattern, replacement)
      }))
      return(data)
    }
    cleanData <- setNa(data, "pValue", Inf)
    cleanData <- setNa(cleanData, "effectSize", -Inf)
    cleanData <- setNa(cleanData, "effectSize", "all pairwise differences equal zero")
    cleanData <- setNa(cleanData, "effectSizeInterpretation", "undefined")
    cleanData <- setNa(cleanData, "pValue", "–")
    cleanData <- setNa(cleanData, "testMethod", "–")
    cleanData <- setNa(cleanData, "effectSize", "–")
    cleanData <- setNa(cleanData, "effectSizeInterpretation", "–")
    cleanData <- replaceSubstring(cleanData, "year", "SoSe ", "")
    cleanData <- replaceSubstring(cleanData, "year", "WiSe ", "")
    cleanData <- cleanData[rowSums(is.na(cleanData)) != ncol(cleanData),]
    return(cleanData)
  }

getEvolutionTableData <- function() {
  filterEvolutionTableRows <- function(data) {
    includedData <- getIncludedData(data)
    ...
    if (includeByEffectSize) {
       additionalRelevantIndices <- which(
         str_detect(includedData$effectSizeInterpretation, fixed(EVOLUTION_SUPPLEMENT_EFFECT_MATCH)))
       relevantIndices <- unique(c(relevantIndices, additionalRelevantIndices))
    }
    relevantData <- includedData[relevantIndices,]
    getOtherTimePointsForQuestions <- function(data) {
      extendedData <- NULL
      for (uni in unique(data$uni)) {
        uni_data <- data[which(data$uni == uni),]
        for (year in unique(uni_data$year)) {
          uni_year_data <- uni_data[which(uni_data$year == year),]
          questionPairs <- unique(uni_year_data[,c("firstQuestionId", "secondQuestionId")])
          for (questionPair in questionPairs) {
            complete_uni_year_data <- includedData[which(
              includedData$uni == uni &
                includedData$year == year),]
            complete_question_data <- complete_uni_year_data[which(
              includedData$firstQuestionId %in% questionPair |
                includedData$secondQuestionId %in% questionPair),]
            extendedData <- rbind(extendedData, complete_question_data)
          }
        }
      }
      return(extendedData)
    }
    extendedRelevantData <- unique(rbind(
        getOtherTimePointsForQuestions(relevantData[which(relevantData$year == HPI_FIRST_YEAR),]),
        includedData[which(includedData$year == HPI_SECOND_YEAR | includedData$year == TUM_COMPLETE_YEAR),]))
    getAverageDifferences <- function(data) {
      averageText <- data$averageValues
      getAverageValue <- function(averageText, index) {
        averageValues <- unlist(str_split(averageText, ", "))
        return(as.numeric(averageValues[index]))
      }
      firstAverages <- unlist(lapply(averageText, function(averageText) { getAverageValue(averageText, 1) }))
      secondAverages <- unlist(lapply(averageText, function(averageText) { getAverageValue(averageText, 2) }))
      return(secondAverages - firstAverages)
    }
    extendedRelevantData$averageDifference <- getAverageDifferences(extendedRelevantData)
    return(extendedRelevantData)
  }

  sortEvolutionTable <- function(data) {
    sortColumns <- c("comparison", "uni", "year", "questionTitle", "secondQuestionId", "firstQuestionId")
  ...
  tableData <- cleanEvolutionTableData(tableData)
  write.csv(tableData, file.path(COMPLETE_RESULTS_DIRECTORY, "evolution_table.csv"), row.names = FALSE)
  return(tableData)
}

printChangeCounts <- function(tableData, alpha = 0.05) {
  changes <- list()
  for (rowIndex in 1:nrow(tableData)) {
...
EVOLUTION_TABLE_DATA <- getEvolutionTableData()
printChangeCounts(EVOLUTION_TABLE_DATA, alpha = 0.05)
printChangeCounts(EVOLUTION_TABLE_DATA, alpha = 0.1)

printAverageAverageChanges <- function(data) {
  includedData <- cleanEvolutionTableData(data)
  for (uni in unique(includedData$uni)) {
    uni_data <- includedData[which(includedData$uni == uni),]
    for (year in unique(uni_data$year)) {
      uni_year_data <- uni_data[which(uni_data$year == year),]
      average_change <- uni_year_data$averageDifference
      average_change <- average_change[which(!is.nan(average_change))]
      average_change <- unlist(lapply(average_change, abs))
      print(paste0("Average average change for ", uni, " in ", year, ": ", mean(average_change)))
    }
  }
}
printAverageAverageChanges(getIncludedData(EVOLUTION_STATS))
printAverageAverageChanges(EVOLUTION_TABLE_DATA)

# How to imoport to Excel:
# - Select "Delimited"
# - Select UTF-8 as file origin
# - "Next"
# - Select comma as separator and " as text qualifier
# - "Next"
# - In "Advanced..." set decimal separator to .
```
...
getOpenEndedData <- function() {
  source("assets/combineResultData.R", local = TRUE)
  getQuestionResponses <- function(openQuestions, resultData, optionData) {
    questionResponses <- list()
    for (questionIndex in 1:nrow(openQuestions)) {
      question <- openQuestions[questionIndex,]
      responses <- list()
      responseIndex <- 1
      for (resultIndex in 1:nrow(resultData)) {
        response <- resultData[resultIndex,]
        text <- response[[question$id]]
        if (!is.null(text) && text != "") {
          if (question$id == "DU12_01") {
            dependend_question <- "DU11"
            dependend_response <- response[[dependend_question]]
            dependend_text <- optionData$MEANING[which(
              optionData$VAR == dependend_question & optionData$RESPONSE == dependend_response)]
            text <- paste0("(Previous answer: ", dependend_text, ") ", text)
          }
          responses[[responseIndex]] <- list(
            "questionnaire" = getCleanQuestionnaireNames(response$QUESTNNR),
            "uni" = getUniFromQuestionnaire(response$QUESTNNR),
            "course.year" = getCourseYear(response$QUESTNNR, optionData, response),
            "text" = text)
          responseIndex <- responseIndex + 1
        }
      } 
      questionResponses[[question$id]] <- list("question" = question$text, "responses" = responses)
    }
    return(questionResponses)
  }
  questionResponses <- getQuestionResponses(
    COMPLETE_QUESTIONS[which(COMPLETE_QUESTIONS$type == "OPEN"),], COMPLETE_RESULTS, COMPLETE_OPTIONS)
  combination_config <- getCombinationConfig(COMPLETE_DATA_DIRECTORY)
  missing_question_ids <- combination_config[["removeQuestionIds"]]
  for (data_directory in c(INITIAL_VERSION_DIRECTORIES, CONFLICTING_VERSION_DIRECTORIES)) {
    question_data <- getQuestions(dataPath = data_directory)
    result_data <- getResultData(dataPath = data_directory)
    option_data <- getOptionData(dataPath = data_directory)
    open_questions <- question_data[which(question_data$id %in% missing_question_ids & question_data$type == "OPEN"),]
    if (nrow(open_questions) > 0) {
      moreResponses <- getQuestionResponses(open_questions, result_data, option_data)
      questionResponses <- append(questionResponses, moreResponses)
    }
  }
  question <- c()
  uni <- c()
  ...
  course.year <- c()
  response.text <- c()
  for (questionId in names(questionResponses)) {
    questionData <- questionResponses[[questionId]]
    responses <- questionData[["responses"]]
    for (response in responses) {
      question <- c(question, questionData[["question"]])
      uni <- c(uni, response[["uni"]])
      questionnaire <- c(questionnaire, response[["questionnaire"]])
      course.year <- c(course.year, response[["course.year"]])
      response.text <- c(response.text, response[["text"]])
    }
  }
  openQuestions <- data.frame(question, uni, questionnaire, course.year, response.text)
  write.csv(openQuestions, file.path(COMPLETE_RESULTS_DIRECTORY, "open_questions.csv"), row.names = FALSE)
  return(openQuestions)
}
OPEN_QUESTIONS <- getOpenEndedData()
View(OPEN_QUESTIONS)
```
