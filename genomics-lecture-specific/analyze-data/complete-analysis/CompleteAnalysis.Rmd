...
getEvolutionTableData <- function(option = 1) {
  filterEvolutionTableRows <- function(data) {
    ...
    # (3) Adding (1) only for first HPI year and all last year responses: 234 (w/ effect size ??)
      ...
      for (uni in unique(data$uni)) {
        ...
        for (year in unique(uni_data$year)) {
          ...
          for (question in unique(uni_year_data$questionTitle)) {
            ...
            extendedData <- rbind(extendedData, complete_data)
          }
        }
      }
      return(extendedData)
    ...
    if (option == 1) {
      extendedRelevantData <- getOtherTimePointsForQuestions(relevantData)
    } else if (option == 2) {
