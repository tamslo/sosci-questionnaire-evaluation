...
getCompleteDataPath <- function() {
  ...
  if (!dir.exists(completeDataPath)) {
    stop("[ERROR] Expecting 'data/AllData' to be present (with combination config)")
  }
...
combineCompleteResultData <- function() {
  ...
  for (rowIndex in 1:nrow(results)) {
    ...
    if (!str_detect(questionnaire, fixed("_"))) {
      CLEAN_QUESTNNR <- c(CLEAN_QUESTNNR, questionnaire)
    } else if (questionnaire == "TUM_Retrospective") {
      CLEAN_QUESTNNR <- c(CLEAN_QUESTNNR, "Q4")
    } else { # questionnaire name with uni name and underscore
      CLEAN_QUESTNNR <- c(CLEAN_QUESTNNR,
                          str_remove(str_remove(str_remove(questionnaire, TUM_NAME), HPI_NAME), "_"))
    }
    # Get COURSE_YEAR
...
getEvolutionStats <- function() {
    ...
    createStackedBarplotsForUniAndYear <- function(parameters) {
      ...
      for (questionName in names(plotParameters)) {
        ...
        for (questionnaire in names(plotParameters[[questionName]][["question.list"]])) {
          ...
          colnames(questionResponses) <- c("CLEAN_QUESTNNR", questionName)
          responses <- rbind(responses, questionResponses)
        }
        responses <- table(responses[[questionName]], responses$CLEAN_QUESTNNR)
        
        # Get statistics
        comparisons <- getPairwiseComparisons(colnames(responses))
        statsStrings <- c()
        for (comparisonName in names(comparisons)) {
          firstQuestionnaire <- comparisons[[comparisonName]][1]
          secondQuestionnaire <- comparisons[[comparisonName]][2]
          population <- c()
          value <- c()
          ...
          comparisonStatistics[[paste(questionId, comparisonName)]] <- c(title, comparisonName, stats, values)
      ...
      colnames(comparisonStatistics) <- c("question.text", "comparison", statisticColumnNames, "values")
  ...
  evolution_stats <- data.frame(questionTitle, uni, year, comparison, isPaired, isBinary, pValue, effectSize, effectSizeInterpretation, values)
...
getEvolutionTableData <- function() {
  tableData <- EVOLUTION_STATS[which(
