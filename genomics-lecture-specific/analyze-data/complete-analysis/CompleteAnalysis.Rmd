...
getEvolutionStats <- function() {
  ...
  for (statsKey in names(evolutionStatsParams)) {
    ...
    if (paired) {
      ...
      avgDiff <- c(avgDiff, stats[["average_differences"]])
    } else {
      ...
      avgDiff <- c(avgDiff, rep("–", nrow(stats)))
    }
  }
  evolution_stats <- data.frame(questionTitle, uni, year, comparison, firstQuestionId, secondQuestionId, isPaired, isBinary, pValue, testMethod, effectSize, effectSizeInterpretation, values, avgDiff, alternativePValue, alternativeTestMethod, alternativeEffectSize, alternativeEffectSizeInterpretation)
...
getEvolutionTableData <- function() {
  filterEvolutionTableRows <- function(data) {
    ...
    relevantData <- includedData[which(
      includedData$pValue != "–" &
        includedData$pValue <= EVOLUTION_SUPPLEMENT_THRESHOLD
      ),]
    # For relevant data, also add other time points for comparison (independent relevant or not)
    # Only for current questionnaire (uni and year): 84
    # Adding also other unis and year: 96
    extendedRelevantData <- NULL
    onlyPerQuestionnaire <- FALSE
    if (onlyPerQuestionnaire) {
